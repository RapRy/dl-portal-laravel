$("document").ready(function () {
    class Notification {
        ...
        static domValidate(elem, hint, errorsArr, whichField) {
            ...
            $(elem)
                .parent()
                .append(`<small class="formHint errorHint">${hint}</small>`);
    ...
    class CustomSelectMenu {
        ...
        loadCustomSelectMenu() {
            ...
            $.each(this.selectOptions, function (i, opt) {
                ...
                $(".customSelectOptions").append(
                    `<span class="customOption">${$(opt).text()}</span>`
                );
            });
        ...
        events() {
            $(".customSelectContainer").on("click", (e) => {
                ...
                if (selectMenuHeight == 0) {
                    // show dropdown
                    this.showCustomSelectMenu(
                        this.heightSelectMenu,
                        e.currentTarget
                    );
                } else {
                    // hide dropdown
    ...
    class ValidateForm {
        ...
        checkImageDimension(resolve) {
            ...
            if (
                ...
                rd.onload = function (e) {
                    ...
                    img.onload = function () {
                        if (
                            (this.height > 25 && this.width > 25) ||
                            (this.height < 25 && this.width < 25)
                        ) {
                            // check image if height and width is not equal to 25px
                            Notification.domValidate(
                                "#customFileIcon",
                                "Category icon dimension must be 25x25px",
                                errors,
                                "categoryIcon"
                            );
        ...
        checkErrors() {
            ...
            if (
                ...
                const imgExt = $("#customFileIcon")[0]
                    .files[0].name.toLowerCase()
                    .split(".");

                if (
                    imgExt[1] != "png" ||
                    $("#customFileIcon")[0].files[0].type != "image/png"
                ) {
                    // if file type is not png
                    Notification.domValidate(
                        "#customFileIcon",
                        "Category Icon must be png file extension",
                        errors,
                        "categoryIcon"
                    );
                } else {
                    ...
                    if (xhr.status == 200) {
                        if (data.hasOwnProperty("error")) {
                            ...
                            if (data.error == "icon") {
                                ...
                                Notification.domValidateAjax(
                                    "#customFileIcon",
                                    "Something went wrong, please try a different icon",
                                    "categoryIcon"
                                );
                            } else {
                                Notification.domValidateAjax(
                            ...
                            `;
                            Notification.domNotificationSuccess(
                                ".notif-container",
                                notifContent,
                                "#catAddedNotifCloseBtn"
                            );
                            // remove selected attrib on the original option
                            $.each(
                                $(".customSelectMenu").children(),
                                function (i, opt) {
                                    $(opt).attr("selected", false);
                                }
                            );
                            ...
                            $.each($(".customOption"), function (i, opt) {
                                $(opt)
                                    .removeClass("customOptionSelected")
                                    .find("i")
                                    .remove();
                            });
                            ...
                            $("#customFileLabel").text(
                                "Only png are allowed.(Dimension 25x25px"
                            );
                        }
                    }
                },
